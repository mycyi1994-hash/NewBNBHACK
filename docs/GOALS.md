...
```
/loop 다음 골을 조건이 모두 PASS가 될 때까지 계속 진행한다. 매 회차 끝에 GOAL STATUS 블록을 쓰고, 모든 조건이 PASS이고 커밋까지 끝났으면 "GOAL COMPLETE"라고 쓰고 루프를 종료한다. 턴 상한에 도달하면 "GOAL STOPPED: 이유"라고 쓰고 루프를 종료한다. --- [여기에 G0~G9 본문 그대로] ---
```
