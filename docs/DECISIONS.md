...
| D-06 | 서버 리전 프랑크푸르트. 암스테르담·런던·도쿄·싱가포르 금지. **확정(9/24 02:30 UTC, G2)**: 워커는 Fly.io `fra` 한 리전·머신 1대(`ijaro-agent`, 재시작 always), DB는 Neon(eu-central-1, Frankfurt). ams·lhr·nrt·sin 미사용. 개발은 한국 PC에서 계속(Q-01) | 제한 지역·차단 보고; 실측: fra 호스트 서명 호출 200/code 0(RWA 목록 350 ms, 가격 배치 253 ms, 지역 코드 없음), 같은 키를 한국에서 36초 뒤 호출해도 40303 없음(Q-01) | M0-04 결과 |
...
| ID | 질문 | 검증 방법 | 결과 | 분기 |
| --- | --- | --- | --- | --- |
| Q-01 | 한국 회선에서 Web3 API 도달? (40304 여부) | M0-04 `pnpm reach` KR/FRA/ICN | **최종(9/24, G2)**: (a) 한국 개발 PC 도달 OK(00:17·00:37·02:33 UTC, 서명 200, 지역 코드 없음). (b) 프랑크푸르트 Fly `fra` 머신 `d8de470f023428`에서 `pnpm reach`(02:31:42 UTC, REGION_TAG=fra): 미서명 401/40101 356 ms, 서명 RWA 목록 200/0 350 ms, 가격 배치 200/0 253 ms, 시계 차 10 ms, 지역 코드 없음 — api_calls `region=fra` 33행(02:30:35–02:31:43 UTC), 429 0건. (c) 같은 키를 프랑크푸르트 워커가 쓰는 중(마지막 호출 02:32:27 UTC)에 한국 PC에서 `pnpm reach` 1회(02:33:02 UTC): 401/40101, 200/0, 200/0 — **40303 없음**. 단 두 지역의 호출이 같은 초에 겹친 것은 아님(36초 간격), 장시간 병행은 미관측. (d) Vercel icn1은 미측정(웹 배포 전). **결정: 서버는 fra(D-06), 한국 PC의 API 호출은 일회성 확인만**(40303 "concurrent multi-region access" 위험 회피). 문서(9/23): 제한 지역 목록에 KR 없음; 40301 지역·40302 프록시/VPN·40303 다중 지역·40304 DeFi 컴플라이언스 | 차단 시 개발도 프랑크푸르트 경유(SSH 터널 또는 원격 개발) |
...
| Q-06 | `referencePrice`의 정의(독립 시세 vs 온체인 파생) | 문서 + 정규장 실측 비교 | **실측(9/24 00:55 UTC)**: `referencePrice = tokenPrice ÷ tokenToShareRatio` — 테이프 9종 모두 상대 오차 ≤ 5.4e-10 → 온체인 파생 **확정**, 독립 시세 아님. 따라서 SPEC §5.5 괴리 가드(`onchain/reference − 1`)는 항상 ≈0으로 의미 없음. **미해결: 대체 기준은 사람 결정**(후보: underlying-market `marketData` — 독립 시세인지 미확인, 또는 발행사 간 가격 비교). 문서(9/23): 커넥터 설명 "derived from the on-chain token price". **후보(문서 확인, 9/24 05:21 UTC)**: 독립 US 주가는 공개 bapi RWA Dynamic V2 `stockInfo.price`("May be `null` outside trading hours" — Skills Hub `binance-tokenized-securities-info/SKILL.md` API 5). Web3 API 문서(llms-full.txt)에는 없음. 엔진은 이 값이 있을 때만 괴리를 판정한다(SPEC §5.5 v2, 프리미엄 2% 초과만 보류). 한국 PC·서버에서 실측 1회 필요 | 파생이면 UI에 "플랫폼 참조가"로 표기, 괴리 임계 완화 |
